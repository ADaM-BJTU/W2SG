...
from weak_to_strong.loss import logconf_loss_fn, product_loss_fn, mse_loss_fn
...
def main(
    ...
    transfer_loss: Union[str, Sequence[str]] = "logconf,mse",
    n_docs: int = 10000,
    n_test_docs: int = 200,
    ...
    train1_ds = load_from_disk(train1_name).select(range(1000))
    train2_ds = load_from_disk(train2_name).select(range(1000))
    test_ds = load_from_disk(test_name)
    
    print("len(train1):", len(train1_ds), "len(train2):", len(test_ds))
    ...
    def train_model(
        ...
        *,
        ...
        train_ds = tokenize_dataset(train_ds, tokenizer, max_ctx, weight=0)
        test_ds = tokenize_dataset(test_ds, tokenizer, max_ctx, weight=0)
        train_ds.save_to_disk("./sciq/gradient_boost/train1_10000_0/")
        if inference_ds:
            inference_ds = tokenize_dataset(inference_ds, tokenizer, max_ctx, weight=0)
            inference_ds.save_to_disk("./sciq/gradient_boost/train2_5000_0/")
    ...
    weak_test_results, weak_ds = train_model(
        ...
        loss_type="mse",
        label="weak",
        subpath=os.path.join("gradient_boost_weak_model", weak_model_size.replace("/", "_")) + str(0),
        lr=weak_lr,
    ...
    with open(
        os.path.join(
            results_folder,
            f"gradient_10000_{weak_model_size.replace('/', '_')}.results_summary.json",
        ),
        "w",
    ) as f:
        json.dump(
