...
class mse_loss_fn(LossFnBase):
    ...
    def __call__(
        ...
        The mean of the cross entropy loss.
        """
        indexs = torch.argmax(labels, dim=-1).unsqueeze(1).to(logits.device)
        labels = labels.gather(dim = -1, index = indexs)

        if step_frac == -1:
            indexs = (indexs == 0)
            labels[indexs] = 1 - labels[indexs]
        else:
            indexs0 = (indexs == 0)
            indexs1 = (indexs == 1)
            labels[indexs0] = -2 * self.logits[indexs0]
            labels[indexs1] = 2 * (1 - self.logits[indexs1])
