...
from weak_to_strong.loss import xent_loss, weight_xent_loss, mse_loss_fn
...
def train_model(
    ...
    while step < nsteps:
        ...
        if isinstance(loss_fn, xent_loss):
            ...
            loss = loss_fn(all_logits, all_labels, step_frac=step / nsteps)
        elif isinstance(loss_fn, mse_loss_fn):
            if None in all_weights:
                loss = loss_fn(all_logits, all_labels, -1)
            else:
                all_weights = torch.stack(all_weights).reshape(-1, 1)
                loss_fn.logits = all_weights
                loss = loss_fn(all_logits, all_labels, 0)
        loss_tot += loss.item()
        loss.backward()
