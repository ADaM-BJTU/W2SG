...
def generate_result_and_prob(txt, knowledge, tokenizer, model):
...
def main(model_path, data_path, data_type):
    ...
    test_data = datasets.load_from_disk(os.path.join(data_path, data_type))
    write_data = []
    gt_labels = []
    predict_labels = []
    ...
    for idx in tqdm(range(len(test_data))):
        ...
        response,prob = generate_result_and_prob(line['txt'],knowledge, tokenizer, model) 
        ...
        json.dump(write_data, open(f"SO_{data_type}_logits.json", "w", encoding="utf-8"), ensure_ascii=False,indent=4)
        print('-------------------')
