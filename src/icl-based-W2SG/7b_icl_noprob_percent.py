...
def encode_prompt(test_line, train_ds, examples,error_num=0): 
    ...
    if examples: 
        ...
        examples = "\n\n".join([ctx_template.format(txt=train_ds[exp]['txt'], hard_label=train_ds[exp]['hard_label']^1) if ii in error_id \
                                else ctx_template.format(txt=train_ds[exp]['txt'], hard_label=train_ds[exp]['hard_label']) for ii,exp in enumerate(examples) ])
        prompt = \
    f'''There is a science knowledge question, followed by an answer. Respond with 1 if the answer is correct, and with 0 otherwise.
...
'''
        ...
        prompt = f'''There is a science knowledge question, followed by an answer. Respond with 1 if the answer is correct, and with 0 otherwise.\n{test_line['txt']}\n'''
        error_id=[]
    # print(prompt)
...
def main(
        batch_size: int = 32, 
        weak_model_size: str = "model_path_qwen-1.8B-chat", 
        strong_model_size: str = "model_path_qwen-7B",     
        icl_process_batch_size: int = 4,
        results_folder: str = "./tmp/results_icl_new",    
        retriever_name: str = 'stochastic_random',#"qwen_topk",  #change stochastic_random
        ice_num: int = 5, 
    ...
    def icl_model( 
        ...
        retriever_name: str,
        ice_num: int = 5, 
        type_: str in ["native", "support"] = "native", 
        error_num: int = 0,
        ):
        ...
        opencil_dataset = DatasetReader(dataset_dict, input_columns=['txt',"hard_label","question"], output_column="hard_label",input_template=template.context_template  )
        if "qwen" in retriever_name : # so_topk_with_sample_selection
        ...
        elif 'so' in retriever_name: # so_with_sample_selection
        ...
        try:
            write_data = json.load(open(f"{save_path}/noprob_{percent}%_{type_}.json","r", encoding="utf-8"))
        except:
            write_data = []
        ...
        for idx,line in enumerate(tqdm(test_data[len(write_data):])): 
            ...
            json.dump(write_data,
                        open(f"{save_path}/noprob_{percent}%_{type_}.json","w", encoding="utf-8"),
                        indent=4,
                        ensure_ascii=False)
    ...
    # # load dataset
    weak_native_ds = [{**line,'hard_label':line['response']} for line in json.load(open(weak_ds_path))]
    weak_support_ds = [{**line,'hard_label':line['response']} for line in json.load(open(weak_support_ds_path))]
